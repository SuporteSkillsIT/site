...
class Order extends AbstractPdf
{
    ...
    protected function _drawItem($item, $page, $order)
    {
        ...
        {
            $lines[0][] = ['text' => $item->getPopSupplierSku(), 'feed' => $this->_skuFeed, 'align' => 'left'];
        }
        else
        {
            $lines[0][] = ['text' => $item->getPopSku(), 'feed' => $this->_skuFeed, 'align' => 'left'];
        ...
        if ($item->getPopSupplierSku())
        {
            $page->drawText($item->getPopSku(), $this->_skuFeed, $this->y - 13, 'UTF-8');
        }
