...
class PaymentsGrid extends \Magento\Backend\Block\Widget\Grid\Extended
{
    ...
    protected function _prepareCollection()
    {
        ...
        $invTable   = $connection->getTableName('bms_supplier_invoice');
        $supplierTable   = $connection->getTableName('bms_supplier');
        $collection->getSelect()
            ->join(
                ['supplier_invoice' => $invTable],
                'main_table.bsip_invoice_id = bsi_id',
                array('bsi_sup_id', 'bsi_reference')
            )
            ->join(
                ['supplier' => $supplierTable],
                'supplier_invoice.bsi_sup_id = supplier.sup_id',
                array('sup_currency')
            );
