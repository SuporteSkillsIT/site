...
class AttributesPrices extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    /**
     * @var \Wyomind\Framework\Helper\Module|null
     */
    ...
    public function price($model, $options, $product, $reference)
    {
        ...
        if ($item->getSpecialFromDate() && !$item->getSpecialToDate()) {
            ...
            if ($item->getSpecialFromDate() <= $this->coreDate->date('Y-m-d H:i:s')) {
                if ($item->getTypeID() == 'bundle') {
                    if (($item->getPriceType() || (!$item->getPriceType() && $item->getSpecialPrice() < $item->getPrice()))
        ...
        } elseif ($item->getSpecialFromDate() && $item->getSpecialToDate()) { // "From date" and "To date" defined
            //Valid promotion date
    ...
    public function salePriceEffectiveDate($model, $options, $product, $reference)
    {
        ...
        if ($offsetHours > 0) {
            $sign = '+';
            $offset = str_pad(abs(floor($offsetHours)), 2, 0, STR_PAD_LEFT)
                . '' . str_pad((abs($offsetHours) - floor(abs($offsetHours))) * 60, 2, 0, STR_PAD_LEFT);
        } else {
            $sign = '-';
            $offset = str_pad(abs(floor($offsetHours)), 2, 0, STR_PAD_LEFT) . ''
                . str_pad((abs($offsetHours) - floor(abs($offsetHours))) * 60, 2, 0, STR_PAD_LEFT);
    ...
    public function applyTax($model, $priceOrig, $priceIncludeTax, $taxClassId, $vat = false)
    {
        ...
        if ($vat === false) { // $vat=false -> automatic
            // If multiple VAT, return the price without tax
            if (!$priceIncludeTax && isset($rates[$taxClassId])) {
                if (count($rates[$taxClassId]) > 1) {
                    return $price;
                } else { // Unique tax > calculation of the price including tax
                    return $price * ($rates[$taxClassId][0]['rate'] / 100 + 1);
                }
            } else {
                return $price;
            }
        } elseif ($vat === "0") { // $vat=='0' -> exclude vat
            if ($priceIncludeTax && isset($rates[$taxClassId])) { // case 1: price including tax > extract it
                if (count($rates[$taxClassId]) > 1) { // Multiple VAT > return price without tax
                    return $price;
                } else { // Unique tax > remove it from the price
                    return 100 * $price / (100 + ($rates[$taxClassId][0]['rate']));
                }
            } else { // case 2: price without tax
                return $price;
            }
        } else { // $vat==true -> force the tax
            if (is_numeric($vat)) { // $vat is_numeric
                if ($taxClassId != 0) { // if tax calculation forces on taxed product
                    return $price * ($vat / 100 + 1);
                } elseif ($taxClassId == 0) { // if tax calculation forced on product without tax
                    return $price;
                }
            } else { // $vat is_string
                $vat = explode('/', $vat);
