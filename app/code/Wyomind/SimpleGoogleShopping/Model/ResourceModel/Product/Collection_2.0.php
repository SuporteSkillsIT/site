...
class Collection extends \Magento\Catalog\Model\ResourceModel\Product\Collection
{
    ...
    {
        parent::__construct(
            $entityFactory, $logger, $fetchStrategy, $eventManager, $eavConfig, $resource, $eavEntityFactory, $resourceHelper,
            $universalFactory, $storeManager, $moduleManager, $catalogProductFlatState, $scopeConfig, $productOptionFactory,
            $catalogUrl, $localeDate, $customerSession, $dateTime, $groupManagement, $connection
        );
        $this->_rowId = $moduleHelper->moduleIsEnabled('Magento_Enterprise') ? 'row_id' : 'entity_id';
    }
    ...
    public function getMainRequest($storeId, $websiteId, $notLike, $concat, $manageStock, $listOfAttributes, $categoriesFilterList, $condition, $params)
    {
        ...
        $this->getSelect()->joinLeft(
            ['stock' => $tableCsi], 'stock.product_id=e.entity_id',
            ['qty' => 'qty', 'is_in_stock' => 'is_in_stock', 'manage_stock' => 'manage_stock',
                'use_config_manage_stock' => 'use_config_manage_stock',
                'backorders' => 'backorders', 'use_config_backorders' => 'use_config_backorders'
            ]
        );
        $this->getSelect()->joinLeft(
            ['url' => $tableCur],
            "url.entity_id=e.entity_id" . $notLike . " AND url.entity_type ='product' AND url.store_id=" . $storeId . " and url.redirect_type = 0",
            ["request_path" => $concat . "(DISTINCT request_path)"]
        );
