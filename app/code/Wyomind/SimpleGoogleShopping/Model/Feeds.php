...
class Feeds extends \Magento\Framework\Model\AbstractModel
{
    ...
    /**
     * @var null|\Wyomind\SimpleGoogleShopping\Helper\Data
     */
    ...
    /**
     * @var \Magento\Tax\Model\ResourceModel\Calculation|null
     */
    ...
    /**
     * @var null|\Wyomind\SimpleGoogleShopping\Helper\Parser
     */
    public $parserHelper = null;
    /**
     * @var null|\Wyomind\SimpleGoogleShopping\Helper\Attributes
     */
    ...
    /**
     * @var null|\Wyomind\SimpleGoogleShopping\Helper\Requirements
     */
    ...
    /**
     * @var \Magento\Framework\View\Asset\Repository|null
     */
     ...
     * @param \Magento\Tax\Model\ResourceModel\Calculation $taxCalculation
     * @param \Magento\Eav\Model\ResourceModel\Entity\Attribute\Option\CollectionFactory $attributeOptionValueCollectionFactory
     * @param ResourceModel\Product\CollectionFactory $productCollectionFactory
     * @param ResourceModel\Functions\CollectionFactory $functionCollectionFactory
     ...
     * @param \Wyomind\SimpleGoogleShopping\Helper\Data $dataHelper
     * @param \Wyomind\SimpleGoogleShopping\Helper\Requirements $requirementsHelper
     * @param \Wyomind\SimpleGoogleShopping\Helper\Attributes $attributesHelper
     * @param ResourceModel\TaxClassFactory $taxClassResourceModelFactory
     * @param ResourceModel\ImagesFactory $imagesResourceModelFactory
     * @param ResourceModel\InventoryStockFactory $inventoryStockResourceModelFactory
    ...
    public function __construct(
        ...
        \Wyomind\SimpleGoogleShopping\Helper\Data $dataHelper,
        \Wyomind\SimpleGoogleShopping\Helper\Requirements $requirementsHelper,
        \Wyomind\SimpleGoogleShopping\Helper\Attributes $attributesHelper,
        ...
        /** @delegaation off */
        /* default args / need to be at the end, because default value */
     ...
     * @throws \Exception
     */
    public function generateXml($request = null)
    {
        try {
            ...
            $mainCollection = $this->productCollectionFactory->create()->getMainRequest(
                $this->params['store_id'],
                $this->storeManager->getStore()->getWebsiteId(), $notLike, $concat, $this->manageStock, $this->listOfAttributes,
                $this->categoriesFilterList, $this->_condition, $this->params
            );
            ...
            if ($this->limit != INF && $this->limit > 0) { // if limit is set
                $this->_counter = $this->limit;
            } else {
                $this->_counter = $this->productCollectionFactory->create()->getProductCount(
                    $this->params['store_id'], $this->storeManager->getStore()->getWebsiteId(),
                    $notLike, $concat, $this->manageStock, $this->listOfAttributes,
                    $this->categoriesFilterList, $this->_condition, $this->params
                );
            ...
            while ($currentLoop < $loops) {
                ...
                $this->progressHelper->log(
                    ">> Fetching products from " . ($this->_sqlSize * ($currentLoop - 1) + 1) . " to " . $limitTo
                    . "  - iteration #$currentLoop", !$this->isPreview, \Wyomind\Framework\Helper\Progress::PROCESSING, round(100 * $i / $this->_counter));
                ...
                foreach ($mainCollection as $product) {
                    ...
                    if ($this->isPreview) {
                        ...
                        if ($inc % (int)$this->licenseHelper->getStoreConfig("simplegoogleshopping/system/buffer") == 0) {
                            ...
                            $timeStart = time();
                            $this->progressHelper->log(
                                $this->inc . "/" . $this->_counter . " items added", !$this->isPreview,
                                \Wyomind\Framework\Helper\Progress::PROCESSING, round(100 * $i / $this->_counter)
                            );
                        }
                    }
    ...
    public function openDestinationFile()
    {
        ...
        $this->_ioWrite->create($this->getSimplegoogleshoppingPath()); // create path if not exists
    ...
    private function extractConfiguration()
    {
        ...
        $this->defaultCurrency = $this->licenseHelper->getStoreConfig("currency/options/base", $this->params['store_id']); // $this->currency->getConfigDefaultCurrencies();
    ...
    private function extractCategories()
    {
        ...
        $listOfCategories = $this->categoryFactory->create()->getCollection()
            ->setStoreId($this->params["store_id"])
            ->addAttributeToSelect(["name", "store_id", "is_active", "include_in_menu"]);
        $this->categories = [];
        ...
        foreach ($result as $infos) {
            foreach ($infos as $info) {
                ...
                switch ($info["property"]) {
                    ...
                    case 'is_in_stock':
                    case 'qty':
                        if ($info["object"] == "configurable" || $this->params["simplegoogleshopping_type_ids"] == "*" 
                            || strpos($this->params["simplegoogleshopping_type_ids"], "configurable") !== false) {
    ...
    private function extractAttributeList($typeId)
    {
        ...
        $attributeLabels->setStoreFilter($this->params["store_id"])
            ->addOrder("option_id", \Magento\Framework\Data\Collection\AbstractDb::SORT_ORDER_ASC)
            ->addOrder("tdv.store_id", \Magento\Framework\Data\Collection\AbstractDb::SORT_ORDER_ASC)
            ->getData();
