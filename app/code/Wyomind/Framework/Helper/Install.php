...
//$this->output->writeln("<comment>Magento version : </comment>" . $version);
...
// composer or zip?
//$composer = strstr($currentScript, "app/code") !== FALSE;
//$this->output->writeln("<comment>Composer        : </comment>" . ($composer ? "Yes" : "No"));
// module path
            $path = str_replace(["Setup" . DIRECTORY_SEPARATOR . "UpgradeData.php", "Setup" . DIRECTORY_SEPARATOR . "Recurring.php"], ["", ""], $currentScript);
//$this->output->writeln("<comment>Module path     : </comment>" . $path);

            foreach ($files as $file) {
                $fullFile = $path . str_replace("/", DIRECTORY_SEPARATOR, $file);
                $ext = pathinfo($fullFile, PATHINFO_EXTENSION);

//$this->output->writeln("<comment>Checking File   : </comment>" . $fullFile);
                foreach ($possibleVersion as $v) {
//$this->output->write(" > Rewrite for  : " . $v . "?");
                    $newFile = str_replace("." . $ext, "_" . $v . "." . $ext, $fullFile);
                    if (file_exists($newFile)) {
//$this->output->writeln(" <info>Yes</info>");
                        $result = copy($newFile, $fullFile);
                        if (!$result) {
//$this->output->writeln("<error> > Error: Rewriting (from " . basename($newFile) . " to " . basename($fullFile) . ")</error>");
                        }
                        break;
                    } else {
//$this->output->writeln(" <error>No</error>");
                    }
                }
            }
        } catch (\Exception $e) {
