...
mgCRMapp.factory('singleFollowuprderService', 
            ['$rootScope', '$resource', '$http',
    function ($rootScope , $resource,    $http) 
{
    
    var container = $resource($rootScope.settings.config.apiURL + '/lead/:resource_id/followups/getSingle/:id/json', {id:'@id', resource_id:'@resource_id'}, 
    {
        query: {
            method: 'GET', 
            cache: false,
...
//                data.date = moment(data.date).unix();
        ...
        get: {
            url: $rootScope.settings.config.apiURL + '/lead/:resource_id/followups/:id/json',
            method: 'GET', 
            isArray: false,
            cache: false,
            responseType: 'json',
