...
angular.module("mgCRMapp").controller(
        'emailTemplatesListController',
        ['$rootScope', '$scope', '$translate', '$http', 'blockUI', 'isAllowed', 'ngDialog', '$state',
function( $rootScope,   $scope,   $translate,   $http,   blockUI,   isAllowed,   ngDialog,   $state)
{
    
    $scope.emailTemplatesTableBlock = blockUI.instances.get('emailTemplatesTable');
    ...
    $scope.callServer = function callServer(tableState) 
    {
        ...
        $scope.emailTemplatesTableBlock.start();
        ...
        $http.post($rootScope.settings.config.apiURL + '/emailtemplates/list/json', params).then(function(result) 
        {
            // update controller container for data from response
            $scope.displayed = result.data.data;
            // stop blockui indicator    
            $scope.emailTemplatesTableBlock.stop();
    ...
    $scope.triggerDeleteEmailTemplate = function(id)
    {
        ...
        $scope.confirmDeleteDialog = ngDialog.openConfirm({
            ...
            // come on give me data from backend
            $http.delete($rootScope.settings.config.apiURL + '/emailtemplates/delete/'+ id +'/json', {
                cache: false,
                isArray: true
            }).then(function(result) 
            {
                ...
                $scope.scopeMessages.push({
                    type:    'success',
                    title:   "Success!",
                    content:  'Email Template has been deleted',
                });
