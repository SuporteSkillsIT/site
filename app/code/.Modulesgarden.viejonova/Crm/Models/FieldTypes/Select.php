...
/**
 * this class handle 'Select' data field's
 */
class Select extends FieldTypeAbstract implements FieldTypeInterface
{
    ...
    public function validate($params = array())
    {
        foreach ($this->field->validators as $v)
        {
            if($v->type == 'required' && empty($params)) {
                ...
                if(count($params) > (int)$v->value) {
                    throw new Exception(sprintf('You can select only %d options', $v->value));
                }
            }
        }
    ...
    function setData($newValue)
    {
        if( is_array($newValue) ) {
            ...
            throw new Exception('Invalid value for select data type');
        }
