...
function crm_for_magento_2_dot_0_license_886()
{   
    $connectionDetails = include dirname(dirname(dirname(dirname(__DIR__)))) . '/etc/env.php';
    $con = mysqli_connect($connectionDetails['db']['connection']['default']['host'], $connectionDetails['db']['connection']['default']['username'], $connectionDetails['db']['connection']['default']['password'], $connectionDetails['db']['connection']['default']['dbname']);

    $results = array('status' => "Unknown Error", 'description' => '');
    $crm_for_magento_2_dot_0_licensekey = "";
    if (!file_exists(dirname(__DIR__) . DIRECTORY_SEPARATOR . 'license.php')) {
        $results['status'] = 'Error';
        $results['description'] = 'CRM For Magento 2.0: Unable to find license.php file. Please rename file license_RENAME.php to license.php';
        return $results;
    }

    require dirname(__DIR__) . DIRECTORY_SEPARATOR . 'license.php';
    $licensekey = $crm_for_magento_2_dot_0_licensekey;
    $valuePath = 'modulesgarden/localkey/crm_for_magento_2_dot_0';

    $key = mysqli_query($con, 'SELECT value FROM core_config_data where path = ' . $valuePath);

    if($key){
    $localkey = mysqli_fetch_row($key);
    }
    else {
        $localkey = null;
    }
    ...
    $licensing_secret_key = 'c1e305e2cf26ed20f083e3a735703d77';
    ...
    if (isset($results['localkey']) && $results['localkey'] != '') {
        mysqli_query($con, 'INSERT INTO core_config_data  ( `scope`, `scope_id`, `path`, `value`) VALUES ( "default", 0, "'.$valuePath.'", "' . $results['localkey'] . '")');
    }
