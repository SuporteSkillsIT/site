...
if (!class_exists(__NAMESPACE__ . '/loader')) {
    ...
    class loader
    {
        ...
        function __construct($dir = null)
        {
            if (empty($dir)) {
                $checkDirs = array(
                    'app' . DS . 'code' . DS . 'community' . DS
                );

                self::$myName = str_replace('\\', '/', __NAMESPACE__);
                ...
                if (self::$whmcsDir) {
                    foreach ($checkDirs as $dir) {
                        ...
                        file_put_contents('system.log', $tmp);
                        if (file_exists($tmp)) {
        ...
        static function loader($className)
        {
            ...
            if ($foundFile) {
                ...
                $error['message'] = 'Unable to load Class:' . $originClassName;
        ...
        static function listClassesInNamespace($className)
        {
            ...
            if ($handle = opendir($foundFile)) {
                while (false !== ($entry = readdir($handle))) {
                    if (
                            $entry != "." && $entry != ".." && strpos($entry, '.php') === (strlen($entry) - 4)
                    ) {
