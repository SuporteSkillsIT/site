...
class Finder extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    public function importUniversal($finder, $file)
    {
        ...
        if (empty($file['name'])) {
            if ($finder->getData('importuniversal_clear') && $finderId) {
    ...
    public function addConditionToProductCollection(
        ...
        if ($isUniversal) {
            ...
            $entityIds = $connection->fetchPairs($connection->select()
                ->from($collection->getTable('amasty_finder_map'), ['id', 'sku'])->where('value_id IN(?)', $ids));
