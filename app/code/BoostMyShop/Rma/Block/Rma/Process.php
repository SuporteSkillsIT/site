...
class Process extends \Magento\Backend\Block\Template
{
    ...
    public function getJsonData()
    {
        ...
        $data['items'] = [];
        $data['shipping'] = $this->getRma()->getOrder()->getshipping_amount() + $this->getRma()->getOrder()->getshipping_tax_amount();
        foreach($this->getRma()->getAllItems() as $rmaItem)
        {
            ...
            $item['price'] = $this->getRefundPrice($rmaItem);
            $data['items'][] = $item;
        }
    ...
    public function getRefundPrice($rmaItem)
    {
        //consider discount in refundable price
        return ($rmaItem->getOrderItem()->getrow_total_incl_tax() - $rmaItem->getOrderItem()->getdiscount_amount()) / $rmaItem->getOrderItem()->getqty_ordered();
    }
